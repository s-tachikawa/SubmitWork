...
// 指定したFBXファイルを読み込む
bool FbxDecoder::Decode( std::string in_path, int in_flame )
{
	...
	//	このモデルのアニメーションカウントの保存
	saveData->animeCount = in_flame;
...
bool FbxDecoder::CreateNode( KFbxNode* pNode, int parentId )
{
	...
	if( attrib != NULL )
	{
		...
		if( type == KFbxNodeAttribute::eMESH )
		{
			...
			KTime time;
			for( int i = 0; i < saveData->animeCount + 1; i++ )
			{
				time.SetTime( 0, 0, 0, i, 0, KTime::eFRAMES30, 0 );//そのフレームに対する時間
				// 指定したフレーム時間の姿勢行列を取得
	...
	if( boneCount > 0 )
	{
		...
		for( int i = 0; i < boneCount; i++ )
		{
			...
			for(int k = 0; k < iNumIndex; k++ )
			{
				...
				blend[ piIndex[ k ] ].weight[ readCount[ piIndex[ k ] ] ] = ( float )pdWeight[ k ];

				if( readCount[ piIndex[ k ] ] < 3 )
				{
					readCount[ piIndex[ k ] ]++;
				}
			}
		...
		for( int i = 0; i < mesh->countData.indexCount; i++ )
		{
			...
			weight[ i ] = math::Vector4(
				mesh->blending[ i ].weight[ 0 ],
				mesh->blending[ i ].weight[ 1 ],
				mesh->blending[ i ].weight[ 2 ],
				mesh->blending[ i ].weight[ 3 ]);
		...
		for( int i = 0; i < saveData->animeCount + 1; i++ )
		{
			time.SetTime( 0, 0, 0, i, 0, KTime::eFRAMES30, 0 );//60フレーム/秒

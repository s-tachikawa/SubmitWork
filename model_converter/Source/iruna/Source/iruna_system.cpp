...
bool System::Setup( U_LONG width, U_LONG height, bool mode, LPCSTR name, U_INT speakers, DWORD style, DWORD styleEx )
{
	if( window && device )
		return false;
	...
	device->SetPresentParameters( width, height, mode );

	if( !device->Create( window->GetHandle() ) )
	{
		return false;
	}
...
bool System::Run()
{
	...
	while( true )
	{
		...
		// もしデバイスがロストしていたら終了する
		if( D3DERR_DEVICELOST == device->GetDeviceState() )
		{
			MessageBox( NULL ,"デバイスロストが発生、アプリケーションを終了します" , NULL ,MB_OK );
			return false;
		}
		break;
	}
